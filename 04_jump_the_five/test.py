...
def test_03():
    """test ful return journey"""

    rv, out = getstatusoutput(f'{prg} "That number to call is 098-765-4321."')
    assert rv == 0
    rv, return_journey = getstatusoutput(
        f'{prg} "That number to call is 512-340-6789."'
    )
    assert rv == 0
    assert return_journey.rstrip() == "That number to call is 098-765-4321."
